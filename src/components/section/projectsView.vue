    ...
    <div class="projects-wrapper p-4 pt-20 flex flex-col items-center">
      ...
      <div class="search-bar flex items-center relative">
        <input
          ...
          style="color: black;"
        />
        <div class="search-btn p-2 absolute flex"><i class="fa-solid fa-magnifying-glass"></i></div>
      </div>
      <div class="project-container mt-12">
          <div v-for="project in filteredProjects" :key="project.title" class="project-card mb-6 px-9 py-7 flex">
            <div class="project-intro flex-1">
              <h2 class="text-lg font-bold text-text-light mb-4">{{ project.title }}</h2>
              <p class="text-sm text-text-light">{{ project.description }}</p>
            </div>
            <div class="project-link flex ">
              <a :href="project.siteUrl" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-globe"></i></a>
              <div class="github-icon pl-3">
                <a :href="project.githubUrl" target="_blank" rel="noopener noreferrer"><img src="/rubyoz-astro/github.svg" alt="GitHub Icon"/></a>
              </div>
            </div>
          </div>
